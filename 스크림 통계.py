...
def compute_stats(g):
    ...
    g = g[[  # 순서 조정
        "총 경기 수", "승률", "평균 KD", "평균 KDA", "전투 점수", "평균 첫 킬", "평균 첫 데스",
        "피해량", "헤드샷%",
        "평균 킬", "평균 데스", "평균 어시스트"
    ]]
    return g
...
if menu == "1. 스트리머별 종합 스탯":
    ...
    cols = ["경기 번호", "날짜", "스트리머 이름", "맵", "사용한 요원", "전투 점수", "KD", "KDA", "피해량", "피해량 격차", "헤드샷%", "첫 킬", "첫 데스", "킬", "데스", "어시스트", "승패"]
...
if menu == "8. 팀별 승률 및 상대전적":
    ...
    if "경기 번호" in df.columns and "팀" in df.columns and "승패" in df.columns:
        ...
        cols = ["경기 번호", "날짜", "스트리머 이름", "팀", "맵", "사용한 요원", "전투 점수", "KD", "KDA",
                "피해량", "피해량 격차", "헤드샷%", "첫 킬", "첫 데스", "킬", "데스", "어시스트", "승패"]
